...
public class BasicEchoTest {
    ...
    public void testBasicEcho() {
        ...
        broker.bind(8080, new EchoServer());
        clientBroker.connect("EchoServer", 8080, new EchoClient(bytesToSend, receivedBytes));
...
class EchoClient implements Broker.ConnectListener {
    ...
    public void connected(Channel channel) {
        for (int i = 0; i < bytesToSend.length; i++) {
            int finalI = i;
            channel.write(bytesToSend, i, 1, bytes -> {
                channel.read(receivedBytes, finalI, 1);
            });
        }
